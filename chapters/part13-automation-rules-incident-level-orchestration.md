...
- **Incident description field behavior.** The `SecurityIncident` table's `Description` field is reported to populate differently — in some cases empty — once Defender XDR generates the incident record instead of Sentinel's native incident-creation logic.
...
| Incident description (`SecurityIncident.Description`) | Populated by Sentinel's native incident-creation logic | Reported to populate differently, sometimes empty, once Defender XDR generates the incident — reverify |
